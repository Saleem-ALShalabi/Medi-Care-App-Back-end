...
async function confirmOrderPayment(orderId, paymentDetails) {
    ...
    const transactionResult = await prisma.$transaction(async (tx) => {
        ...
        const amountPaidCents = Math.round(paymentDetails.amountPaid * 100);

        if (orderTotalCents !== amountPaidCents) {
            // We can also add more debug info to the error for ourselves.
            throw new Error(`The amount paid does not match the order total. Order Total: ${order.totalAmount}, Amount Paid: ${paymentDetails.amountPaid}`);
        }
...
async function updateOrderStatus(orderId, newStatus) {
    ...
    // --- Step 1: Perform all database operations in a transaction ---
    const transactionResult = await prisma.$transaction(async (tx) => {
        const order = await tx.Order.findUnique({
            where: { id: numericOrderId },
            include: {
        ...
        // ... (The existing logic for CANCELLED and DELIVERED remains the same)
    ...
    // --- Step 2: Perform side effects (like sending emails) AFTER the transaction succeeds ---

    // Deconstruct the results from the transaction
